...
import com.bsw.library.utils.TextExtractUtils;
import com.bsw.library.utils.Utils;
...
//    }
     ...
     */
    private void replaceData(List<T> mData, @IntRange(from = 1) int pageIndex, @IntRange(from = 1) int pageSize) {
        int dataSize = Utils.judgeListNull(this.mData);
        int startPosition = pageSize * (pageIndex - 1);
    ...
    int getDataSize() {
        ...
        if ((filterUseState & FILTER_DATA) == 0) {
            return Utils.judgeListNull(mLayoutData);
        } else {
            return Utils.judgeListNull(mShowLayoutData);
        }
    }
    ...
    private void startFilter(boolean isNotify) {
        ...
        if (filterUseState >= FILTER_DATA) {     // 需要根据输入框文本过滤之后刷新
            ...
            filterable.getFilter().filter(TextExtractUtils.getString(filterEt));
