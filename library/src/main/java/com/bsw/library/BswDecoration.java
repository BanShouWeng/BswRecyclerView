...
public class BswDecoration extends RecyclerView.ItemDecoration {
    ...
    public BswDecoration(Context context, @LimitAnnotation.DecorationType int type) {
        this(context, type, Color.parseColor("#F0F0F0"), 1);
    }

    public BswDecoration(Context context, @LimitAnnotation.DecorationType int type, int color, int lineWidth) {
        this.type = type;
        dividerPaint = new Paint();
        //设置分割线颜色
        dividerPaint.setColor(context.getResources().getColor(color));
        //设置分割线宽度
        mydevider = context.getResources().getDimensionPixelSize(lineWidth);
    }
